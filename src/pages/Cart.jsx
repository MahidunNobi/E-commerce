...
cartItems.forEach(i => {
  ...
  return (
    <div>
      <div> 
        { cartItems.length>0 ? 
        cartItems.map(item => <SingleCart key={item.id} id={item.id} img={item.image} title={item.title} itemCount={item.itemsCount} category={item.category} price={item.price} increamentItemCount={increamentItemCount} decreamentItemCount={decreamentItemCount} />)  
        : 
        <h1 className="text-3xl"> You Nothing in Cart</h1> }
    </div>
    <div id="total" className='container mx-auto flex justify-end mb-10'> 
